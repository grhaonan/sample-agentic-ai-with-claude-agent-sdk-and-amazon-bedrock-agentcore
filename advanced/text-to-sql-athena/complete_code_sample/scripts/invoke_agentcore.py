...
Usage:
    # Local dev mode
    uv run python scripts/invoke_agentcore.py --dev "How many students are enrolled?"

    # AWS hosted mode
    uv run python scripts/invoke_agentcore.py "How many students are enrolled?"
"""
...
def invoke_agentcore(query: str, session_id: str, dev_mode: bool = False, claude_sdk_session_id: str = None) -> tuple[str, bool]:
    """
...
Examples:
  ...
  uv run python scripts/invoke_agentcore.py "Show me enrollment trends"

  # Specific query (may need multiple rounds of clarification)
  uv run python scripts/invoke_agentcore.py --dev "Analyze student performance by department"
...
Note: The script automatically handles multiple rounds of clarification questions.
    ...
    if not args.query:
        ...
        print("\nUsage: uv run python scripts/invoke_agentcore.py [--dev] \"<your query>\"")
        sys.exit(1)
